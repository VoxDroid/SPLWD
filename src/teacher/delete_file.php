...
if ($conn->query($sql) === TRUE) {
  ...
  $sql123="INSERT INTO `log` (`log_id`, `date`, `teacher_id`, `action_type`, `details`, `previous`, `updated`, `student_id`, `status`) VALUES (NULL, '".$date3."', '".$_SESSION['teacher_id']."', 'Deleted a File', '".$uploaded."', '', '', '".$_GET['lrn']."', '".$_SESSION['school']."');";
...
$date3 = date('Y-m-d');
  ...
  if ($conn->query($sql) === TRUE) {
    $sql123="INSERT INTO `log` (`log_id`, `date`, `teacher_id`, `action_type`, `details`, `previous`, `updated`, `student_id`, `status`) VALUES (NULL, '".$date3."', '".$_SESSION['teacher_id']."', 'Retrieve a File', '".$uploaded."', '', '', '".$_GET['lrn']."', '".$_SESSION['school']."');";
    if ($conn->query($sql123) === TRUE) {
      echo "<p>6".$type."</p>";
